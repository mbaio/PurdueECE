...
//MGM Grand - Studio Walk Food Court 3799 Las Vegas Blvd S Eastside
#define BUFLEN 9000
...
//     }      
    if (id_num2 == id_num1)
      {
	//printf("id_num2 == id_num1\n");
        List * list_node = List_createNode(id_num1,offset_bus,test_offset);
        tree_node * temp_node = create_node(list_node,line_elements1[1]);
	//time_insert++;
        temp_tree = tree_insert_name(temp_node,temp_tree);   
        //destroyStringArray(line_elements2, file_line_length2);
      }
      test_offset = offset_rev;
...
int cmploc(const void * ptr1,const void * ptr2)
{
  ...
  int outcome = strcasecmp(str1,str2);
  free(str1);
  free(str2);
  return outcome;
}
